...
async def _(context):
    ...
    for i in range(10):
        ...
        assert ret == expected, f"ret: {ret}"
        if i == 1:
            await conn.exec(f"DROP TABLE temp_{i}")

    rows = await context.conn.query_iter("SELECT COUNT(*) FROM system.temporary_tables")
    temp_table_count = list(rows)[0].values()[0]
    assert temp_table_count == 9, f"temp_table_count = {temp_table_count}"

    await conn.close()

    rows = await context.conn.query_iter("SELECT COUNT(*) FROM system.temporary_tables")
    temp_table_count = list(rows)[0].values()[0]
    assert temp_table_count == 0, f"temp_table_count = {temp_table_count}"
